  ...
  return (
    <div className="flex flex-col h-full bg-white p-6 rounded-xl shadow-sm border border-gray-100 text-center items-center justify-center">
      ...
      <p className="text-xs text-gray-500 mb-6 leading-relaxed">
        请在左侧预览图中点击紫色标签修改打印份数。<br/>
        系统将自动改名为 <span className="text-purple-600 font-mono font-bold">1-N</span> 序列。
      </p>
      
      <button 
        onClick={onExecute}
        disabled={selectedCount === 0}
        className="w-full py-4 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white rounded-2xl font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2"
      >
        <span>执行批量复制</span>
