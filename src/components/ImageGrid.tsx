...
export default function ImageGrid({
  images, isDragging, disabled, zoomWidth, setZoomWidth, 
  onToggleSelect, onSelectAll, onDeselectAll, onClearAll, 
  activeTab, replicateCounts, onUpdateCount, onRemoveOne
}: ImageGridProps) {
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-gray-200/50 px-3 py-1.5 rounded-full border border-gray-200/80 shadow-inner">
              ...
              <input disabled={disabled} type="range" min="100" max="350" step="1" value={zoomWidth} onChange={(e) => setZoomWidth(Number(e.target.value))} className="w-20 h-1 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-blue-500 hover:accent-blue-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed" />
            </div>
          </div>
      ...
      {images.length > 0 ? (
        <div className="w-full h-full p-6 overflow-y-auto grid gap-4 auto-rows-max content-start custom-scrollbar" style={{ gridTemplateColumns: `repeat(auto-fill, minmax(${zoomWidth}px, 1fr))` }}>
          {images.map((img, index) => (
            <div key={img.path} onClick={() => { if (!disabled && selectionEnabled) onToggleSelect(index); }} className={`flex flex-col gap-1 group p-2 rounded-2xl transition-all duration-150 border ${selectionEnabled && img.selected ? "bg-blue-50 border-blue-500 shadow-md scale-[0.98]" : !img.isSupported ? "bg-red-50/30 border-red-100 cursor-not-allowed opacity-80" : selectionEnabled ? "bg-transparent border-transparent cursor-pointer hover:border-gray-200 hover:bg-gray-50 hover:scale-[1.02]" : "bg-transparent border-transparent"}`}>
              <div className={`relative aspect-square rounded-xl overflow-hidden border shadow-sm flex items-center justify-center p-3 ${img.isSupported ? 'bg-gray-50 border-gray-200' : 'bg-red-50 border-red-200'}`}>
                ...
                {/* 🌟 内联紫色修改器 */}
                {!disabled && activeTab === 'replicate' && img.isSupported && (
                  <div 
                    onClick={(e) => { e.stopPropagation(); if (!disabled) setEditingPath(img.path); }}
                      className={`absolute bottom-2 left-2 text-[11px] font-bold px-2 py-1 rounded-lg shadow-lg cursor-pointer transition-all z-20 border min-w-[36px] flex justify-center items-center ${editingPath === img.path ? "bg-white text-purple-700 border-purple-500 scale-110" : "bg-purple-600 text-white border-purple-400 hover:bg-purple-700 active:scale-95"}`}
                    >
                      {editingPath === img.path ? (
                      <input
                        autoFocus disabled={disabled} type="number" min={1} defaultValue={replicateCounts?.[img.path] || 1}
                ...
                {selectionEnabled && img.isSupported && (
                ...
                <div className="absolute bottom-2 right-2 z-30">
                  <button onClick={(e) => { e.stopPropagation(); if (!disabled) onRemoveOne?.(img.path); }} className="text-xs text-red-600 bg-white/90 px-2 py-0.5 rounded-md border border-red-100 hover:bg-red-50">移除</button>
                </div>
              </div>
