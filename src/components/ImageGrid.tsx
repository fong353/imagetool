            ...
            <div key={img.path} onClick={() => { if (selectionEnabled) onToggleSelect(index); }} className={`flex flex-col gap-1 group p-2 rounded-2xl transition-all duration-150 border ${selectionEnabled && img.selected ? "bg-blue-50 border-blue-500 shadow-md scale-[0.98]" : !img.isSupported ? "bg-red-50/30 border-red-100 cursor-not-allowed opacity-80" : selectionEnabled ? "bg-transparent border-transparent cursor-pointer hover:border-gray-200 hover:bg-gray-50 hover:scale-[1.02]" : "bg-transparent border-transparent"}`}>
              <div className={`relative aspect-square rounded-xl overflow-hidden border shadow-sm flex items-center justify-center p-3 ${img.isSupported ? 'bg-gray-50 border-gray-200' : 'bg-red-50 border-red-200'}`}>
                ...
                <div className="absolute bottom-2 right-2 z-30">
                  <button onClick={(e) => { e.stopPropagation(); onRemoveOne?.(img.path); }} className="text-xs text-red-600 bg-white/90 px-2 py-0.5 rounded-md border border-red-100 hover:bg-red-50">移除</button>
                </div>
              </div>
